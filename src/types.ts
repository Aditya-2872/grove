...
export interface MetricWidget extends WidgetBase {
  ...
  direction?: "up" | "down";
  log?: LogEntry[];
  /** How often the value starts over. Set it for anything period-scoped ("Words
   *  today", "Workouts this week") — without it the ring reads 500/500 on day
   *  two and the app greets you saying the day is already done. The reset is
   *  derived from the log, never written: no timers, no server, no stale
   *  rollover. "total" and absent both accumulate forever ("Total words",
   *  "Saved") — but they differ to the one-time migration: absent means
   *  "predates this field, may need stamping", "total" means "the user chose
   *  to accumulate", so migration never re-fights an explicit choice. */
  period?: MetricPeriod;
}

export type MetricPeriod = "day" | "week" | "month" | "total";
